...
let fetchurl;
var fetchData;
class TestPlanning extends React.Component{
...
fetchData = ()=> {
     var status;
         fetchurl = '/planning/getTestPlanning?location='+localStorage.getItem("locationvalue")+'&project='+localStorage.getItem("projectvalue");
        if(this.props.status!=null){
            status = this.props.status;
            fetchurl = '/planning/getTestPlanningDraft?location='+localStorage.getItem("locationvalue")+'&project='+localStorage.getItem("projectvalue")+'&status='+status;
        }
  ...
  onSubmit= (locationvalue,projectvalue) => {
    ...
    render(){
        ...
        var table = {};
        table.data=[];
        table.columnList = TableColumnMapping.TestPlanning;
       ...
       if(this.props.deleteflag!=null) {
        ...
        table.options = {
            exportCSVText: 'Export TestPlanning',
            deleteText: 'Delete TestPlanning',
            insertText:"Add TestPlanning",
            saveText: 'Save',
            closeText: 'Close',
            afterDeleteRow: function(rowKeys,obj) {
                alert('TestPlanning Deleted' );
                fetchurl="/planning/deleteTestPlanning";

                 fetchApi(fetchurl,JSON.stringify(obj));
               
            },afterInsertRow: function(obj) {

                fetchurl = '/planning/addTestPlanning?location='+localStorage.getItem("locationvalue")+'&project='+localStorage.getItem("projectvalue");
                            ...
                            if(!obj.change){
                                obj[columnName]=obj.oldvalue
                                 ...
                                 if(obj.change){
                                    ...
                                    fetchurl = '/planning/updateTestPlanning';
        ...
        return (
             <div style={{"display":"realtive"}}>
                    {this.state.showResults? <PlanningHeader  title={"Test Planning"} headerRight={headerRight} /> : null}
