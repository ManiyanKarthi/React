...
class SoftwareSpecifications extends React.Component{
...
fetchData = ()=> {
     var status;
         fetchurl = '/planning/getSoftwareSpecifications?location='+localStorage.getItem("locationvalue")+'&project='+localStorage.getItem("projectvalue");
        if(this.props.status!=null){
            status = this.props.status;
            fetchurl = '/planning/getSoftwareSpecificationsDraft?location='+localStorage.getItem("locationvalue")+'&project='+localStorage.getItem("projectvalue")+'&status='+status;
        }
  ...
  onSubmit= (locationvalue,projectvalue) => {
    ...
    render(){
        ...
        var table = {};
        table.data=[];
        table.columnList = TableColumnMapping.SoftwareSpecifications;
        ...
        table.options = {
            exportCSVText: 'Export SoftwareSpecifications',
            deleteText: 'Delete SoftwareSpecifications',
            insertText:"Add SoftwareSpecifications",
            saveText: 'Save',
            closeText: 'Close',
            afterDeleteRow: function(rowKeys,obj) {
                alert('SoftwareSpecifications Deleted' );
                             fetchurl = '/planning/deleteSoftwareSpecifications';
                
               fetchApi(fetchurl,JSON.stringify(obj));

            },afterInsertRow: function(obj) {

                   fetchurl = '/planning/addSoftwareSpecifications?location='+localStorage.getItem("locationvalue")+'&project='+localStorage.getItem("projectvalue");
        ...
        table.cellEdit = {mode: 'dbclick',blurToSave: true,afterSaveCell: (obj,columnName,newValue) => { 
              if(obj.oldvalue!=newValue) {
                            if(!obj.change){
                             ...
                             fetchurl = '/planning/updateSoftwareSpecifications';
                  fetchApi(fetchurl,JSON.stringify(obj));
        ...
        return (
            <div style={{"display":"realtive"}}>
                    {this.state.showResults? <PlanningHeader  title={"Software Specification"}  headerRight={headerRight} /> : null}
...
export default SoftwareSpecifications;