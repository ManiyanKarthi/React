...
class SeatingInformation extends React.Component{
...
fetchData = ()=> {
          var status;
         fetchurl = '/planning/getSeatingInformation?location='+localStorage.getItem("locationvalue")+'&project='+localStorage.getItem("projectvalue");
        if(this.props.status!=null){
            status = this.props.status;
            fetchurl = '/planning/getSeatingInformationDraft?location='+localStorage.getItem("locationvalue")+'&project='+localStorage.getItem("projectvalue")+'&status='+status;
        }
...
componentWillReceiveProps(){
    ...
    render(){
        ...
        var table = {};
        table.data=[];
        table.columnList = TableColumnMapping.SeatingInformation;
       ...
       if(this.props.deleteflag!=null) {
        ...
        table.options = {
            exportCSVText: 'Export SeatingInformation',
            deleteText: 'Delete SeatingInformation',
            insertText:"Add SeatingInformation",
            saveText: 'Save',
            closeText: 'Close',
            afterDeleteRow: function(rowKeys,obj) {
                alert('SeatingInformation Deleted' );
                                fetchurl = '/planning/deleteSeatingInformation';
                    fetchApi(fetchurl,JSON.stringify(obj));

            },afterInsertRow: function(obj) {
                  fetchurl = '/planning/addSeatingInformation?location='+localStorage.getItem("locationvalue")+'&project='+localStorage.getItem("projectvalue");
        ...
        table.cellEdit = {mode: 'dbclick',blurToSave: true,afterSaveCell: (obj,columnName,newValue) => { 
               if(obj.oldvalue!=newValue) {
                            if(!obj.change){
                             ...
                             fetchurl = '/planning/updateSeatingInformation';
        ...
        return (
            <div style={{"display":"realtive"}}>
                    {this.state.showResults? <PlanningHeader  title={"Seating Information"} headerRight={headerRight} /> : null}
