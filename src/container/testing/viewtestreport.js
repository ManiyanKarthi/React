...
constructor(props){
       ...
       this.state= {successMesure:false,showResults:false,typeoftest:"--Select--",locationvalue:"--Select--",projectvalue:"--Select--",locOptionlist:[],projectOptionlist:[],projectValidationError:false,locValidationError:false}
 ...
 componentWillMount() {
    ...
    handleClick = (obj) => {
     ...
     getPreviewData = (projectDetails)=>{
        let fetchurl = '/testing/getCommunicationtestplanning?location='+this.state.locationvalue+'&project='+this.state.projectvalue+'&reportstatus=Approved'+'&projectDetails='+projectDetails+'&typeoftest='+this.state.typeoftest;
     ...
     getplanComments = (location,project) =>{
        if(location!="--Select--" && project!="--Select--") {
             fetch('/testing/getTestplanComments?location='+location+'&project='+project+'&typeoftest='+this.state.typeoftest+'&projectDetails='+this.state.projectDetails).then(res => res.json()).then(data =>{
             if(data.length>0){
                        this.setState({
                                plancomments:data,
     ...
     onSubmit = () => {
             ...
             let fetchurl = '/testing/getCommunicationtestplanning?location='+this.state.locationvalue+'&project='+this.state.projectvalue+'&reportstatus=Approved'+'&typeoftest='+this.state.typeoftest;
            ...
            fetch(fetchurl).then(res => res.json()).then(data =>{
                        this.setState({data:data.map((obj) =>{
                           ...
                           obj.projectDetails= obj.projectDetails;
                            return obj;
                        }),showResults:true,preview:false,successMesure:true});                                
                });
